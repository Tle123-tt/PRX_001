...
const updateCart = asyncHandler(async (req, res) => {
  const { _id } = req.user;
  const { pid, quantity, language } = req.body;
  if (!pid || !quantity || !language) throw new Error("Missing imputs");
  const user = await User.findById(_id).select();
  const alreadyProduct = user?.cart?.find((el) => el.product === pid);
  if (alreadyProduct) {
    if (alreadyProduct.language === language) {
      ...
      const response = await User.findByIdAndUpdate(_id, {
        $push: { cart: { product: pid, quantity, language } },
      });
      return res.status(200).json({
        success: response ? true : false,
